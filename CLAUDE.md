...
- **Search results are scoped to requested paths**: Like ripgrep, `vecgrep query src/` only returns results from `src/`, not the entire index. Running from a subdirectory without explicit paths scopes results to that subdirectory — `cd src && vecgrep "query"` only shows `src/` results. The `SearchScope` struct (in `types.rs`) groups `explicit_paths` and `path_scopes`. Path scoping is a post-filter using `Path::starts_with`; scopes of `"."` or empty (at the project root) mean "no filtering." Consistent across CLI, TUI, and `--serve`.
