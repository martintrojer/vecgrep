...
- **Search results are scoped to requested paths**: Like ripgrep, `vecgrep query src/` only returns results from `src/`, not the entire index. The `SearchScope` struct (in `types.rs`) groups `explicit_paths` and `path_scopes`. Path scoping is a post-filter using `Path::starts_with`; scopes of `"."` or empty mean "no filtering." Consistent across CLI, TUI, and `--serve`.
