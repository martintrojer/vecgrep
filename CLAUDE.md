...
- **`--open-cmd` for TUI file opener**: Template string with `{file}` and `{line}` placeholders, e.g. `"nvim +{line} {file}"`. Configurable via CLI, project config (`open_cmd`), or global config. Falls back to `$PAGER` or `less` when not set. The command string is split on whitespace after placeholder expansion.
